...
def get_thresholds_closeness(num_graph, edgedata, graph_ind):
    '''
    ...
    for graph_id in range(1, num_graph + 1):
        ...
        for v in id_location:
            try:
                sp = path_length(graph, v)
                totsp = sum(sp.values())
                closeness = 0.0
                if totsp > 0.0:
                    closeness = 1 / totsp
                thresh_list.append(closeness)
            except:
                continue
...
def closeness_sub(num_graph, edgedata, graph_ind, threshold_array):
    '''
    This function calculates the vector of number of 0-cells and number of 1-cells for sub-level closeness filtration
    ...
    for graph_id in range(1, num_graph + 1):
        ...
        for degr in threshold_array:
            Rindex = [index for (index, degree) in closn if degree <= degr]
...
def closeness_super(num_graph, edgedata, graph_ind, threshold_array):
    '''
    This function calculates the vector of number of 0-cells and number of 1-cells for super-level closeness filtration
    function.The parameters are:
